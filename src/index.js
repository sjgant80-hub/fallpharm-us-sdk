...
// Extracted from fallpharm-us/index.html · 140359 bytes of source logic
...
// Sovereign prescription management + US law engine for State Bar-regulated firms
...
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Pharmacy opinion signed and retained per State Bar 13.5',{id:a.id,prescriptionId:a.prescriptionId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallpharm</strong> is a tool for State Bar-regulated US attorneys. It assists with prescription management, CDD, document generation, and State Bar Accounts Rules tracking. It is <strong>not regulatory submission or pharmacy opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <p class="lead">Sovereign prescription management + US law research for 1-10 person State Bar-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>attorney</option><option>partner</option><option>parapharmacy</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function tabAdvice(m){
 ...
 return `<div class="card"><h3>Issue pharmacy opinion <span class="meta">State Bar 13.5 retention · 6yrs</span></h3>
...
async function askT3(){
 ...
 const sys='You are a US pharmacy research assistant for State Bar-regulated attorneys. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated pharmacy advice — research only. Always say "verify against primary sources".';
