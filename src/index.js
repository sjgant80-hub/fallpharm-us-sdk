// fallpharm-us SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from fallpharm-us/index.html · 140278 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "fallpharm-us" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Fallpharm v1.0.0 · prime 743 · MIT
// Sovereign prescription management + UK law engine for SRA-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='fallpharm-us';const VERSION='1.0.0';const PRIME=743;const STORE='fallpharm-us';const CONFIG_V='fallpharm-us@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','prescriptions','advice','corpus','weaves','audit','settings'];
...
const TABS=[
 ...
 {id:'prescription',name:'Prescription',ico:'§'},
...
const T0_RULES=[
 ...
 {q:'CFA vs DBA?',a:'CFA (Conditional Fee Agreement): "no win, no fee" with success fee uplift (max 100% of base costs, capped at 25% of damages excl. future care in PI). Governed by Courts & Pharmacy Services Act 1990 s.58, CFA Order 2013. DBA (Damages-Based Agreement): payment is % of damages recovered (50% civil, 35% employment, 25% PI). DBA Regs 2013. CFA success fee recoverable from client only since LASPO 2012. Hybrid CFA/DBA unenforceable: Zuberi v Lexlaw [2021] EWCA Civ 16.'},
 ...
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order pharmacy representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Pharmacy Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
...
const CORPUS=[
 ...
 {id:'lsa2007',title:'Pharmacy Services Act 2007',area:'other',summary:'Modernised pharmacy services regulation; created Pharmacy Services Board, LeO, ABS structures; defined reserved pharmacy activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to pharmacy advice (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',prescriptionTab:'overview',activePrescriptionId:null,
 firm:null,advisers:[],clients:[],prescriptions:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.prescriptions=(await idbAll('prescriptions'))||[];
 ...
 if(meta){state.activePrescriptionId=meta.activePrescriptionId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.prescriptionTab=meta.prescriptionTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activePrescriptionId:state.activePrescriptionId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,prescriptionTab:state.prescriptionTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const prescriptionId=(payload&&payload.prescriptionId)||state.activePrescriptionId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+prescriptionId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,prescriptionId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function savePrescription(m,reason,noBc){m.updatedAt=Date.now();await idbPut('prescriptions',m);if(!state.prescriptions.find(x=>x.id===m.id))state.prescriptions.push(m);else state.prescriptions=state.prescriptions.map(x=>x.id===m.id?m:x);await audit('prescription.updated',reason||'Prescription updated',{id:m.id,prescriptionId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('prescription.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Pharmacy opinion signed and retained per SRA 13.5',{id:a.id,prescriptionId:a.prescriptionId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,prescriptions:state.prescriptions});
 ...
 for(const mt of(p.prescriptions||[])){const local=state.prescriptions.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await savePrescription(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='prescription.created'||m.type==='prescription.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.prescriptions.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await savePrescription(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 for(const m of state.prescriptions){
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',prescriptionId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
// FACTORIES
// ════════════════════════════════════════════════════════════════
function newFirmRec(){return{id:'fm_'+uid(),createdAt:Date.now(),updatedAt:Date.now(),name:'',tradingName:'',sraNumber:'',companiesHouseNo:'',vatNumber:'',registeredAddress:{line1:'',line2:'',city:'',postcode:'',country:'GB'},piInsurer:'',piPolicyNo:'',piExpiresAt:null,professionalBody:'SRA',brandColor:'#8b1a1a',brandLogoDataUri:'',setupCompletedAt:null,nextPrescriptionRefSeq:1}}
function newAdviserRec(){return{id:'ad_'+uid(),firmId:state.firm&&state.firm.id,createdAt:Date.now(),updatedAt:Date.now(),archivedAt:null,name:'',email:'',phone:'',smcrRole:'solicitor',practicingCertNo:'',practicingCertExpiry:'',cpdHoursThisYear:0,cpdActivities:[],colp:false,cofa:false,status:'active',startedAt:Date.now(),leftAt:null}}
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForPrescription:''}},
...
function newPrescriptionRec(clientId){
 const seq=(state.firm&&state.firm.nextPrescriptionRefSeq)||(state.prescriptions.length+1);
 if(state.firm)state.firm.nextPrescriptionRefSeq=seq+1;
 ...
 title:'New prescription',practiceArea:'civil-litigation',
 responsibleSolicitorId:state.settings.currentAdviserId||'',supervisingPartnerId:'',
...
function activePrescription(){return state.prescriptions.find(m=>m.id===state.activePrescriptionId)}
function prescriptionClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 const mt=newPrescriptionRec(cl.id);
 mt.title='DEMO · Patel v Singh · RTA quantum · overwrite me';
 mt.practiceArea='personal-injury';mt.responsibleSolicitorId=adviserId;mt.status='active';mt.feeArrangement='conditional';mt.cfaSuccessFeePct=25;mt.estimatedFees=12500;
 ...
 mt.demo=true;
 await savePrescription(mt,'demo seed',true);
 state.activePrescriptionId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setPrescriptionTab(id){state.prescriptionTab=id;saveMeta();render()}
function selectPrescription(id){state.activePrescriptionId=id;state.active='prescription';state.prescriptionTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activePrescription();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no prescription selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const prescriptions=state.prescriptions.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(prescriptionClient(m)?clientLabel(prescriptionClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 if(f.area&&m.practiceArea!==f.area)return false;
 if(f.responsible&&m.responsibleSolicitorId!==f.responsible)return false;
 ...
 sb.innerHTML=`
 <h4>Prescriptions · ${state.prescriptions.length}</h4>
 ...
 <button class="btn primary sm" onclick="newPrescription()">+ new prescription</button>
 </div>
 <div class="prescription-list">${prescriptions.length?prescriptions.map(m=>{
 const cl=prescriptionClient(m);
 ...
 return `<div class="prescription-card ${state.activePrescriptionId===m.id?'active':''}" onclick="selectPrescription('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no prescriptions yet · click + new prescription</div>'}</div>`;
}
function render(){
 ...
 else if(a==='prescriptions')v.innerHTML=viewPrescriptions();
 else if(a==='prescription')v.innerHTML=viewPrescription();
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallpharm</strong> is a tool for SRA-regulated UK solicitors. It assists with prescription management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or pharmacy opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Fallpharm</h1>
 <p class="lead">Sovereign prescription management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
 <div class="row"><div class="field"><label>Firm name *</label><input id="f_name" placeholder="Acme Solicitors LLP"></div><div class="field"><label>Trading name</label><input id="f_trading"></div></div>
 <div class="row"><div class="field"><label>SRA number *</label><input id="f_sra" placeholder="123456"></div><div class="field"><label>Companies House</label><input id="f_ch"></div></div>
...
function onbStep2(){
 ...
 <h1>First adviser</h1>
 <p class="lead">Add yourself — SRA roll number and which compliance roles you hold (COLP/COFA mandatory for the firm).</p>
 ...
 <div class="row"><div class="field"><label>SRA roll number *</label><input id="a_sra"></div><div class="field"><label>Practising cert expiry</label><input type="date" id="a_exp"></div></div>
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>parapharmacy</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.prescriptions.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.prescriptions.filter(m=>m.status==='closed').length;
 const critical=state.prescriptions.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.prescriptions){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">SRA ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.prescriptions.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no prescriptions</div>'}
 </div>
 ...
 <div class="kpi"><span class="l">Retention</span><span class="v brass">SRA · 6 years</span></div>
 </div>
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectPrescription('${m.id}')">open →</button></div>`
...
function viewPrescriptions(){
 const mm=state.prescriptions.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Prescriptions · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newPrescription()">+ new prescription</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=prescriptionClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectPrescription('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleSolicitorId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no prescriptions · click + new prescription</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewPrescription(){
 const m=activePrescription();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No prescription selected</h3><p>Choose one from the sidebar, or <a onclick="newPrescription()">create new</a>.</p></div>';
 const cl=prescriptionClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closePrescription('${m.id}')">${m.status==='closed'?'reopen':'close prescription'}</button>
 <button class="btn sm ox" onclick="deletePrescription('${m.id}')">delete</button>
 </div>
 </div>
 <div class="prescription-tabs">${tabs.map(t=>`<button class="${state.prescriptionTab===t[0]?'active':''}" onclick="setPrescriptionTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.prescriptionTab==='overview'?tabOverview(m,cl):''}
 ${state.prescriptionTab==='retainer'?tabRetainer(m):''}
 ${state.prescriptionTab==='conflict'?tabConflict(m,cl):''}
 ${state.prescriptionTab==='timeline'?tabTimeline(m):''}
 ${state.prescriptionTab==='advice'?tabAdvice(m):''}
 ${state.prescriptionTab==='documents'?tabDocs(m):''}
 ${state.prescriptionTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Prescription detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updPrescription('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updPrescription('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updPrescription('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updPrescription('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible solicitor</label><select onchange="updPrescription('responsibleSolicitorId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleSolicitorId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updPrescription('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updPrescription('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updPrescription('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updPrescription('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updPrescription('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updPrescription('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updPrescription('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','pharmacy-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updPrescription('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updPrescription('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updPrescription('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updPrescription('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updPrescription('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updPrescription('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updPrescription('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updPrescription('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local prescriptions + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (prescription)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForPrescription)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this prescription</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updPrescription('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updPrescription('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updPrescription('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.prescriptionId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue pharmacy opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>fallpharm-uspaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>fallpharm-uspractice</strong>. This view shows ledger ref IDs.</p>
...
function viewAdvisers(){
 ...
 <table class="tbl"><thead><tr><th>Name</th><th>Role</th><th>SRA roll</th><th>PC expiry</th><th>COLP</th><th>COFA</th><th>CPD</th><th></th></tr></thead><tbody>
...
function viewFirm(){
 ...
 <div class="row"><div class="field"><label>SRA number</label><input value="${esc(f.sraNumber)}" oninput="state.firm.sraNumber=this.value;saveFirm()"></div><div class="field"><label>Companies House</label><input value="${esc(f.companiesHouseNo)}" oninput="state.firm.companiesHouseNo=this.value;saveFirm()"></div></div>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic pharmacy patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activePrescriptionId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activePrescriptionId}','${w.id}')">apply to active prescription</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
 return disclaimer()+`<div class="section-h"><h2>UK Law Corpus · ${CORPUS.length}</h2><div class="sub">core statutes &amp; codes</div></div>
...
function viewQA(){
 return disclaimer()+`<div class="section-h"><h2>Q &amp; A · UK law research</h2><div class="sub">T0 offline · T3 BYOK cascade</div></div>
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activePrescriptionId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activePrescription().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Audit · P3</h2><div class="sub">${state.audit.length} entries · 6yr retention · SRA 13.5</div><div class="actions"><button class="btn" onclick="exportAudit()">export JSON</button></div></div>
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / prescription.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Prescription</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.prescriptions.find(x=>x.id===a.prescriptionId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updPrescription(field,val){const m=activePrescription();if(!m)return;m[field]=val;savePrescription(m,'field '+field+' updated')}
async function newPrescription(){
 const m=newPrescriptionRec();state.activePrescriptionId=m.id;
 await savePrescription(m,'new prescription');await audit('prescription.created','New prescription created',{id:m.id,prescriptionId:m.id,ref:m.ref});broadcast('prescription.created',m);
 state.active='prescription';state.prescriptionTab='overview';saveMeta();render();
}
async function closePrescription(id){const m=state.prescriptions.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await savePrescription(m,'prescription '+m.status);if(m.status==='closed')broadcast('prescription.closed',m);else broadcast('prescription.reopened',m);render()}
async function deletePrescription(id){if(!confirm('Delete this prescription? (audit log preserved)'))return;await idbDel('prescriptions',id);state.prescriptions=state.prescriptions.filter(m=>m.id!==id);if(state.activePrescriptionId===id)state.activePrescriptionId=null;await audit('prescription.deleted','Prescription deleted',{id});render()}
async function sendClientCare(id){const m=state.prescriptions.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await savePrescription(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.prescriptions.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await savePrescription(m,'event added');render()}
async function addFileRef(id){const m=state.prescriptions.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await savePrescription(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.prescriptions.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await savePrescription(m,'file ref removed');render()}
async function issueAdvice(prescriptionId){
 const m=state.prescriptions.find(x=>x.id===prescriptionId);if(!m)return;
 ...
 const sig=await sha256(prescriptionId+adviserId+ts+txt);
 const a={id:'adv_'+uid(),prescriptionId,clientId:m.clientId,adviserId,ts,text:txt,signature:sig};
 await saveAdvice(a);broadcast('advice.issued',a);toast('opinion signed and retained');$('#advText').value='';render();
}
async function applyWeave(prescriptionId,wid){const m=state.prescriptions.find(x=>x.id===prescriptionId);if(!m)return;m.appliedWeaves=m.appliedWeaves||[];if(!m.appliedWeaves.includes(wid))m.appliedWeaves.push(wid);await savePrescription(m,'weave '+wid+' applied');toast('weave applied');render()}
async function unapplyWeave(prescriptionId,wid){const m=state.prescriptions.find(x=>x.id===prescriptionId);if(!m)return;m.appliedWeaves=(m.appliedWeaves||[]).filter(x=>x!==wid);await savePrescription(m,'weave '+wid+' removed');render()}
function showWeave(wid){const w=WEAVES.find(x=>x.id===wid);if(!w)return;modal(`<h3>${esc(w.name)}</h3><p style="font-style:italic;color:var(--cream-dim);margin-bottom:10px">${esc(w.archetype)}</p><div class="seg"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Move</b><br>${esc(w.move)}</div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Authorities</b><br><span class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</span></div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Counter</b><br>${esc(w.counter)}</div><div style="margin-top:14px">${state.activePrescriptionId?`<button class="btn primary" onclick="applyWeave('${state.activePrescriptionId}','${w.id}');closeModal()">apply to active prescription</button> `:''}<button class="btn ghost" onclick="closeModal()">close</button></div>`)}
async function runConflictCheck(prescriptionId){
 const m=state.prescriptions.find(x=>x.id===prescriptionId);if(!m)return;
 const cl=prescriptionClient(m);
 ...
 await savePrescription(m,'conflict scan run');
...
function editClient(id){const c=state.clients.find(x=>x.id===id);if(!c)return;modal(`<h3>Edit client</h3>
 ...
 <div class="row"><div class="field"><label>DOB</label><input type="date" id="ec_dob" value="${esc(c.dob)}"></div><div class="field"><label>Companies House no</label><input id="ec_ch" value="${esc(c.entityNumber)}"></div></div>
...
function answerT0(q){
 ...
 ans+='\n\n— Fallpharm T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activePrescription();const cl=m?prescriptionClient(m):null;
 const ctx=m?`\nActive prescription: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK pharmacy research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated pharmacy advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Fallpharm T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
async function callOpenAI(sys,user){
 const r=await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json','authorization':'Bearer '+state.settings.openaiKey},body:JSON.stringify({model:'gpt-4o-mini',messages:[{role:'system',content:sys},{role:'user',content:user}],max_tokens:1500})});
...
async function callOpenRouter(sys,user){
 const r=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json','authorization':'Bearer '+state.settings.openrouterKey},body:JSON.stringify({model:'anthropic/claude-3.5-sonnet',messages:[{role:'system',content:sys},{role:'user',content:user}],max_tokens:1500})});
 const j=await r.json();if(j.error)throw new Error(j.error.message);return j.choices[0].message.content;
}
// ════════════════════════════════════════════════════════════════
// EXPORT / WIPE / DEMO
...
function exportAll(){downloadJSON('fallpharm-us-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,prescriptions:state.prescriptions,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('fallpharm-us-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.prescriptions.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('prescriptions',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.prescriptions=state.prescriptions.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activePrescriptionId&&!state.prescriptions.find(m=>m.id===state.activePrescriptionId))state.activePrescriptionId=null;
 await audit('demo.purged','Demo data removed',{prescriptions:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Fallpharm booted',{version:VERSION,prime:PRIME});
})();
